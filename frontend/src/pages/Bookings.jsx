	...
	return (
		<main className={`min-h-[calc(100vh-73px)] px-6 py-12 ${isDark ? "bg-slate-950 text-slate-100" : "bg-white text-slate-900"}`}>
			<section className={`mx-auto max-w-4xl rounded-3xl border p-8 ${isDark ? "border-white/10 bg-white/5 backdrop-blur" : "border-slate-200 bg-slate-50 shadow-sm"}`}>
				...
				<h1 className={`mt-4 text-3xl font-black ${isDark ? "text-white" : "text-slate-900"}`}>Resource Bookings</h1>
				<p className={`mt-2 text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>
					Reserve study spaces, shared rooms, or equipment when those services are enabled.
				</p>
				<div className={`mt-8 rounded-2xl border p-5 ${isDark ? "border-white/10 bg-slate-900/60" : "border-slate-200 bg-white"}`}>
					<p className={`font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>Booking portal</p>
					<p className={`mt-2 text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>This page is ready for booking controls and can be connected to the backend booking module next.</p>
				</div>
			</section>
		</main>
	);
